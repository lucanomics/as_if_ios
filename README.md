...
- 저장소는 현재 `localStorage`입니다(용량 한계 존재). IndexedDB 전환은 Phase 2.
- 개인정보 패턴 감지는 휴리스틱이며 100%를 보장하지 않습니다 — 최종 확인은 사용자 책임입니다.

## 12. 향후 개선 TODO (Phase 2)

- IndexedDB 저장 드라이버로 교체 (storage 추상화 이미 준비됨)
- PWA/오프라인 캐시
- 로컬 앱 잠금(PIN, 자동 잠금, 평문 미저장)
- 로그별 체크리스트 상태 저장
- 통계 시각화(차트)
...
자동화로 직접 검증한 항목(1, 3, 11, 12, 2, 14, 15, 16, 19, 20)은 브라우저에서 확인했으며, 나머지는
동일 로직 경로를 사용하는 UI에서 수동 확인했습니다.
